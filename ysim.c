...
step_ysim(Y86 *y86)
{
  ...
  case RMMOVQ_CODE: ;//OP 4
    Byte regA = get_nybble(read_memory_byte_y86(y86, pc+sizeof(Byte)),1);
    Byte regB = get_nybble(read_memory_byte_y86(y86, pc+sizeof(Byte)),0);
    Word displacement = read_memory_word_y86(y86, pc+(2*sizeof(Byte)));
    Byte valA = read_register_y86(y86, regA);
    Word valB = read_register_y86(y86, regB)+displacement;
  ...
  case MRMOVQ_CODE: ;//OP 5
    ...
    Byte regB = get_nybble(read_memory_byte_y86(y86, pc+sizeof(Byte)),0);
    Word displacement = read_memory_word_y86(y86, pc+(2*sizeof(Byte)));
    Word valA = read_register_y86(y86, regA)+displacement;
    Byte value = read_memory_byte(y86, valA);
    write_register_y86(y86, regB, value);
    write_pc_y86(y86, pc+2*sizeof(Byte)+sizeof(Word));
    break;
  case OP1_CODE: ;//OP 6 - dummy line to allow declarations
    Byte fn = get_nybble(op, 0);         
    Register regA = get_nybble(read_memory_byte_y86(y86, pc+1),1); 
    Register regB = get_nybble(read_memory_byte_y86(y86, pc+1),0);  
